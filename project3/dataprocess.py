def edge_csv():
    f = open('data/0.edges').readlines()
    out = open('import/edges.csv', 'w')
...
def feature_extract():
    feature_file = open('data/0.featnames').readlines()
    feats_file = open('data/0.feat').readlines()
    feat_out = open('import/feat.csv', 'w')
    print('id', file=feat_out, end='')
    features = {}
    length = {}
    for line in feature_file:
        tmp = line.strip().split(' ')
        feature_name, feature_id = tmp[1], tmp[2]
...
def circle():
    circle_file = open('data/0.circles').readlines()
    out = open('import/circles.csv', 'w')
